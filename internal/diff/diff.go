...
func compareProcesses(r *model.DiffResult, a, b model.RuntimeGraph) {
	...
	for _, p := range a.Processes {
		ma[p.Name+"|"+p.Command] = true
	}
	for _, p := range b.Processes {
		mb[p.Name+"|"+p.Command] = true
	}
	for k := range mb {
		if !ma[k] {
			r.Changes = append(r.Changes, model.Change{Surface: "process", Kind: "added", Key: k, After: k, Severity: model.SeverityMedium, Summary: "new runtime process observed: " + humanProcess(k)})
		}
	}
	for k := range ma {
		if !mb[k] {
			r.Changes = append(r.Changes, model.Change{Surface: "process", Kind: "removed", Key: k, Before: k, Severity: model.SeverityLow, Summary: "runtime process no longer observed: " + humanProcess(k)})
		}
	}
}
...
func compareNetwork(r *model.DiffResult, a, b model.RuntimeGraph) {
	...
	for k, n := range ma {
		if _, ok := mb[k]; !ok {
			r.Changes = append(r.Changes, model.Change{Surface: "network", Kind: "removed", Key: k, Before: n, Severity: model.SeverityLow, Summary: "network behavior no longer observed: " + networkHuman(n)})
		}
	}
}

func compareFilesystem(r *model.DiffResult, a, b model.RuntimeGraph) {
	baseline := compactFiles(a.Filesystem)
	candidate := compactFiles(b.Filesystem)
	...
	for k, f := range mb {
		if _, ok := ma[k]; !ok {
			sev := model.SeverityMedium
			if strings.HasPrefix(f.Path, "/etc/") || strings.HasPrefix(f.Path, "/root/") || strings.Contains(f.Path, "secret") || strings.Contains(f.Path, "credential") {
				sev = model.SeverityHigh
			}
	...
	for k, f := range ma {
		if _, ok := mb[k]; !ok {
			r.Changes = append(r.Changes, model.Change{Surface: "filesystem", Kind: "removed", Key: k, Before: f, Severity: model.SeverityLow, Summary: fmt.Sprintf("filesystem mutation no longer observed: %s %s", f.Kind, f.Path)})
		}
	}
}

// compactFiles removes directory-level mutations when a more specific child
// mutation is present. Docker diff commonly reports both a changed parent
// directory and the actual leaf that caused it; keeping only the leaf makes
// compatibility evidence materially easier to understand.
func compactFiles(in []model.FileMutation) []model.FileMutation {
	out := make([]model.FileMutation, 0, len(in))
	for i, f := range in {
...
func comparePrivileges(r *model.DiffResult, a, b model.RuntimeGraph) {
	...
	for c := range newCaps {
		if !oldCaps[c] {
			sev := model.SeverityHigh
			if c == "SYS_ADMIN" || c == "SYS_MODULE" || c == "BPF" || c == "SYS_PTRACE" {
	...
	if a.Privileges.EffectiveUID != b.Privileges.EffectiveUID {
		sev := model.SeverityMedium
		if b.Privileges.EffectiveUID == 0 && a.Privileges.EffectiveUID != 0 {
			sev = model.SeverityCritical
		}
	...
	if a.Image.User != b.Image.User {
		r.Changes = append(r.Changes, model.Change{Surface: "privilege", Kind: "changed", Key: "image_user", Before: a.Image.User, After: b.Image.User, Severity: model.SeverityHigh, Summary: fmt.Sprintf("image runtime user changed from %q to %q", a.Image.User, b.Image.User)})
	}
}
...
func compareLifecycle(r *model.DiffResult, a, b model.RuntimeGraph) {
	...
	if !a.Lifecycle.OOMKilled && b.Lifecycle.OOMKilled {
		r.Changes = append(r.Changes, model.Change{Surface: "lifecycle", Kind: "regression", Key: "oom_killed", After: true, Severity: model.SeverityCritical, Summary: "new version was OOM-killed during the scenario"})
	}
	if a.Lifecycle.ExitCode == 0 && b.Lifecycle.ExitCode != 0 {
		r.Changes = append(r.Changes, model.Change{Surface: "lifecycle", Kind: "regression", Key: "exit_code", Before: 0, After: b.Lifecycle.ExitCode, Severity: model.SeverityCritical, Summary: fmt.Sprintf("new version exits non-zero (%d)", b.Lifecycle.ExitCode)})
	}
}

func networkKey(n model.NetworkEndpoint) string {
	switch n.Direction {
	case "listen":
		return fmt.Sprintf("%s|listen|%d", n.Protocol, n.LocalPort)
	case "inbound":
		// Remote source ports are ephemeral and differ on every replay. The
		// operational contract for inbound traffic is the workload's local
		// listening surface, not the client's transient source port.
		return fmt.Sprintf("%s|inbound|%d", n.Protocol, n.LocalPort)
	default:
		return fmt.Sprintf("%s|%s|%s|%d", n.Protocol, n.Direction, n.RemoteAddress, n.RemotePort)
	}
}

func networkHuman(n model.NetworkEndpoint) string {
	if n.Direction == "listen" {
		return fmt.Sprintf("%s :%d", strings.ToUpper(n.Protocol), n.LocalPort)
	}
	if n.Direction == "inbound" {
		return fmt.Sprintf("%s :%d", strings.ToUpper(n.Protocol), n.LocalPort)
	}
	return fmt.Sprintf("%s %s:%d", strings.ToUpper(n.Protocol), n.RemoteAddress, n.RemotePort)
}

func humanProcess(k string) string {
...
func severityRank(s model.ChangeSeverity) int {
	switch s {
	case model.SeverityCritical:
		return 5
	case model.SeverityHigh:
